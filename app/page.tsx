...
export default function Home() {
  ...
  // AI Provider toggle — switch between LangChain and Vercel AI SDK
  const [aiProvider, setAiProvider] = useState<"langchain" | "vercel">("langchain");

  // Thinking animation — ensures minimum display time for AI feel
  const THINKING_MIN_MS = 4000;
  ...
  async function stopThinking() {
    ...
    const remaining = THINKING_MIN_MS - elapsed;
    if (remaining > 0) {
  ...
  // Handle inline component button actions
  async function handleInlineAction(action: string, payload?: any) {
    ...
    if (action === "confirm_execute" && payload?.pendingKeeper) {
      ...
      const execParams = {
        ...
        target: payload.pendingAction?.target || payload.target,
      };
      ...
      try {
        ...
        if (json.success && json.data.status === "success") {
          ...
          setMessages((prev) => [...prev, {
            ...
            actionData: {
              vaultaction: {
                ...payload, status: "executed", txIds, hashScanLinks,
                toolCalls: json.data.toolCalls, hcsLog: json.data.hcsLog, durationMs: json.data.durationMs,
              },
            },
            timestamp: new Date(),
          }]);
          ...
          setMessages((prev) => [...prev, {
            ...
            charts: ["vaultaction"],
            actionData: { vaultaction: { ...payload, status: "failed", error: json.error || "Execution failed" } },
            timestamp: new Date(),
          }]);
        }
      } catch (err: any) {
      ...
      try {
        ...
        if (json.success && json.data.status === "success") {
          ...
          setMessages((prev) => [...prev, {
            ...
            charts: ["lendingaction"],
            actionData: { lendingaction: { ...payload, status: "failed", error: json.error || "Execution failed" } },
            timestamp: new Date(),
          }]);
        }
      } catch (err: any) {
  ...
  // Detect action commands that need execution
  ...
  const suggestionFetchedRef = useRef<Set<string>>(new Set());

  async function fetchAISuggestions(userMsg: string, assistantMsg: string, msgId: string) {
    if (suggestionFetchedRef.current.has(msgId)) return;
    suggestionFetchedRef.current.add(msgId);

    try {
      const positionSummary = portfolio?.positions?.length
        ? portfolio.positions.map(p => {
            const bits = [];
            if (p.supplied > 0) bits.push(`${p.supplied.toFixed(1)} supplied`);
            if (p.borrowed > 0) bits.push(`${p.borrowed.toFixed(1)} borrowed`);
            return `${p.symbol}: ${bits.join(", ")}`;
          }).join("; ")
        : "No active positions";
      const hfStr = portfolio?.healthFactor ? `Health Factor: ${portfolio.healthFactor.toFixed(2)}` : "";
      const sentimentStr = sentiment ? `Sentiment: ${sentiment.score} (${sentiment.signal})` : "";
      const walletStr = connectedAccount ? `Wallet: ${connectedAccount}` : "No wallet connected";

      const contextPrompt = [
        `USER MESSAGE: "${userMsg}"`,
        `SYSTEM RESPONSE: "${assistantMsg.substring(0, 300)}"`,
        ``,
        `CURRENT STATE:`,
        `- ${walletStr}`,
        `- Positions: ${positionSummary}`,
        hfStr ? `- ${hfStr}` : null,
        sentimentStr ? `- ${sentimentStr}` : null,
        ``,
        `Based on what the user just did, their current positions, and market conditions, suggest the 3 best ACTIONABLE next steps.`,
        `Suggestions must be specific commands like "Deposit 100 HBAR", "Show my positions", "Borrow 5 USDC" — NOT vague advice.`,
        `If user just deposited → suggest borrowing or checking positions.`,
        `If user just borrowed → suggest checking health factor or repaying.`,
        `If user just viewed audit → suggest filtering by type.`,
        `If user asked about strategy → suggest executing it.`,
        `Respond ONLY with JSON: {"action":null,"params":{},"response":"","suggestions":["cmd1","cmd2","cmd3"],"charts":[]}`,
      ].filter(Boolean).join("\n");
      ...
      if (json.success && json.data?.suggestions?.length > 0) {
        const sug = json.data.suggestions.slice(0, 3);
        setAiSuggestions(sug);
        setMessages(prev => prev.map(m =>
          m.id === msgId ? { ...m, suggestions: sug } : m
        ));
        console.log(`[AI Suggestions] ✅ Got: ${sug.join(" | ")}`);
      }
    } catch (e) {
      console.warn("[AI Suggestions] Background fetch failed:", e);
    }
  }
  ...
  useEffect(() => {
    ...
    if (lastMsg.suggestions && lastMsg.suggestions.length > 0) {
      setAiSuggestions(lastMsg.suggestions);
      return;
    }
    const lastUser = [...messages].reverse().find(m => m.role === "user");
    if (!lastUser) return;
    fetchAISuggestions(lastUser.content, lastMsg.content, lastMsg.id);
  ...
  async function handleSend() {
    ...
    try {
      ...
      // Send to agent for AI-powered response (with provider toggle)
      const res = await fetch("/api/agent", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
      ...
      const aiSug = json.data?.suggestions?.length > 0 ? json.data.suggestions.slice(0, 3) : undefined;
      if (aiSug) setAiSuggestions(aiSug);

      const aiCharts: string[] = json.data?.charts || [];
      const mergedCharts = Array.from(new Set([...detectedCharts, ...aiCharts]));
      ...
      const assistantMessage: ChatMessage = {
        ...
        charts: mergedCharts.length > 0 ? mergedCharts as ChartType[] : undefined,
  ...
  // ============================================
  // Render — Bonzo-inspired dark purple theme
  // ============================================
        ...
        <div className="max-w-[1440px] mx-auto px-6 py-3.5 flex items-center justify-between">
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-2.5">
              <div className="w-9 h-9 rounded-xl flex items-center justify-center" style={{ background: "linear-gradient(135deg, #7c3aed 0%, #a855f7 100%)" }}>
                <Brain className="w-5 h-5 text-white" />
              </div>
              <div>
                <h1 className="text-lg font-bold tracking-tight text-white">
                  Vault<span className="text-purple-400">Mind</span>
                </h1>
                <p className="text-[10px] text-purple-400/60 -mt-0.5 font-medium tracking-wide uppercase">
                  AI DeFi Keeper • Hedera
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3">
            {/* AI Provider Toggle */}
            <div className="flex items-center rounded-xl overflow-hidden border border-purple-800/40" style={{ background: "rgba(30, 20, 60, 0.6)" }}>
              <button
                onClick={() => setAiProvider("langchain")}
                className={`flex items-center gap-1.5 text-[11px] font-medium px-3 py-1.5 transition-all duration-200 ${
                  aiProvider === "langchain"
                    ? "text-emerald-300 bg-emerald-500/15"
                    : "text-gray-500 hover:text-gray-400"
                }`}
              >
                <span className={`w-1.5 h-1.5 rounded-full transition-colors ${aiProvider === "langchain" ? "bg-emerald-400" : "bg-gray-600"}`} />
                LangChain
              </button>
              <div className="w-px h-4 bg-purple-800/40" />
              <button
                onClick={() => setAiProvider("vercel")}
                className={`flex items-center gap-1.5 text-[11px] font-medium px-3 py-1.5 transition-all duration-200 ${
                  aiProvider === "vercel"
              ...
              >
                <span className={`w-1.5 h-1.5 rounded-full transition-colors ${aiProvider === "vercel" ? "bg-blue-400" : "bg-gray-600"}`} />
                Vercel AI
              </button>
            </div>

            {connectedAccount && (
              <div className="flex items-center gap-1.5 text-[11px] text-purple-300 rounded-xl px-3 py-1.5 border border-purple-700/30" style={{ background: "rgba(139, 92, 246, 0.08)" }}>
                <Wallet className="w-3 h-3" />
                {connectedAccount}
              </div>
            )}
            {autoLoop && (
              <div className="flex items-center gap-1.5 text-[11px] text-emerald-300 rounded-xl px-3 py-1.5 border border-emerald-700/30" style={{ background: "rgba(16, 185, 129, 0.08)" }}>
            ...
            <a
              ...
              className="flex items-center gap-1.5 text-[11px] text-purple-400/70 hover:text-purple-300 rounded-xl px-3 py-1.5 border border-purple-800/30 hover:border-purple-700/50 transition-all" style={{ background: "rgba(30, 20, 60, 0.4)" }}
            >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 h-full">
          ...
          <div className="lg:col-span-1 space-y-4 overflow-y-auto max-h-[calc(100vh-120px)] pr-1 scrollbar-thin">
            ...
            <div style={{
              ...
              boxShadow: sidebarPulse.has("wallet") ? "0 0 30px rgba(139,92,246,0.4), 0 0 60px rgba(139,92,246,0.15)" : "none",
            ...
            <div style={{
              transition: "box-shadow 0.5s ease-in-out, transform 0.3s ease-in-out",
              boxShadow: sidebarPulse.has("keeper") ? "0 0 30px rgba(139,92,246,0.4), 0 0 60px rgba(139,92,246,0.15)" : "none",
            ...
            <div style={{
              transition: "box-shadow 0.5s ease-in-out, transform 0.3s ease-in-out",
              boxShadow: sidebarPulse.has("positions") ? "0 0 30px rgba(139,92,246,0.4), 0 0 60px rgba(139,92,246,0.15)" : "none",
            ...
            <div style={{
              transition: "box-shadow 0.5s ease-in-out, transform 0.3s ease-in-out",
              boxShadow: sidebarPulse.has("sentiment") ? "0 0 30px rgba(251,191,36,0.3), 0 0 60px rgba(251,191,36,0.1)" : "none",
            ...
            <div style={{
              transition: "box-shadow 0.5s ease-in-out, transform 0.3s ease-in-out",
              boxShadow: sidebarPulse.has("vaults") ? "0 0 30px rgba(168,85,247,0.4), 0 0 60px rgba(168,85,247,0.15)" : "none",
            ...
            {/* Market Overview */}
            <MarketCard markets={markets} loading={marketLoading} error={marketError} />

            {/* Decision History */}
            {keeperHistory.length > 0 && (
              <div style={{
                transition: "box-shadow 0.5s ease-in-out, transform 0.3s ease-in-out",
                boxShadow: sidebarPulse.has("history") ? "0 0 30px rgba(168,85,247,0.4), 0 0 60px rgba(168,85,247,0.15)" : "none",
            ...
            {/* Agent Status */}
            <div className="rounded-xl border border-purple-900/30 p-4" style={{ background: "rgba(15, 12, 30, 0.6)" }}>
              ...
              <div className="space-y-2.5 text-xs">
                ...
                <div className="flex justify-between">
                  ...
                  <span className={`font-medium ${aiProvider === "vercel" ? "text-blue-400" : "text-emerald-400"}`}>
                    {aiProvider === "vercel" ? "Vercel AI SDK" : "LangChain"}
                  </span>
                </div>
                <div className="flex justify-between">
          ...
          <div className="lg:col-span-2 flex flex-col gap-0">
            {/* Tab Bar — Bonzo-style */}
            <div className="flex items-center gap-1 rounded-t-xl border border-b-0 border-purple-900/30 px-3 pt-2.5" style={{ background: "rgba(15, 12, 30, 0.5)" }}>
              {([
                ...
                <button
                  ...
                  className={`flex items-center gap-1.5 text-xs font-medium px-4 py-2.5 rounded-t-lg transition-all duration-200 ${
                    activeTab === tab.id
                      ? "text-purple-300 border border-b-0 border-purple-700/40"
                      : "text-gray-500 hover:text-gray-300"
                  }`}
                  style={activeTab === tab.id ? { background: "rgba(139, 92, 246, 0.08)" } : {}}
                >
              ...
              {activeTab === "chat" && (
                <div className="flex flex-col min-h-[550px] h-full">
                  ...
                  <div className="px-5 py-3 border-b border-purple-900/20 flex items-center gap-2" style={{ background: "rgba(15, 12, 30, 0.4)" }}>
                    ...
                    <span className="ml-auto text-[10px] text-purple-500/60 font-medium">
                      Hedera Agent Kit + Bonzo Plugin • {aiProvider === "vercel" ? "Vercel AI SDK" : "LangChain"}
                    </span>
                  </div>
                  ...
                  <div className="flex-1 overflow-y-auto px-5 py-4 space-y-4">
                    {messages.map((msg) => (
                      ...
                      >
                        {msg.role === "assistant" && (
                          <div className="w-7 h-7 rounded-lg flex items-center justify-center flex-shrink-0 mt-0.5" style={{ background: "linear-gradient(135deg, rgba(124, 58, 237, 0.3) 0%, rgba(168, 85, 247, 0.2) 100%)", border: "1px solid rgba(139, 92, 246, 0.2)" }}>
                        ...
                        <div
                          className={`rounded-xl px-4 py-3 text-sm leading-relaxed ${
                            msg.role === "user"
                              ? "max-w-[75%] text-gray-200 border border-purple-500/25"
                              : `${msg.charts?.length ? "max-w-[95%] w-full" : "max-w-[85%]"} text-gray-300 border border-purple-900/25`
                          }`}
                          style={
                            msg.role === "user"
                          ...
                          {msg.toolCalls && msg.toolCalls.length > 0 && (
                            <div className="mt-2 pt-2 border-t border-purple-900/20">
                              ...
                              {msg.toolCalls.map((tc, i) => (
                                <span
                                  key={i}
                                  className="inline-block text-[10px] text-purple-400/80 rounded px-1.5 py-0.5 mr-1 mb-1"
                        ...
                        {msg.role === "user" && (
                          <div className="w-7 h-7 rounded-lg flex items-center justify-center flex-shrink-0 mt-0.5 border border-purple-800/30" style={{ background: "rgba(30, 20, 60, 0.5)" }}>
                  ...
                  {/* Input — Bonzo-style */}
                  <div className="px-5 py-4 border-t border-purple-900/20" style={{ background: "rgba(15, 12, 30, 0.4)" }}>
                    <div className="flex gap-2.5">
                      <input
                        ...
                        className="flex-1 rounded-xl px-4 py-3 text-sm text-gray-200 placeholder:text-gray-600 focus:outline-none focus:ring-1 focus:ring-purple-500/40 border border-purple-800/30 focus:border-purple-600/50 transition-all"
                      ...
                      <button
                        ...
                        className="px-5 py-3 rounded-xl text-sm font-medium transition-all duration-200 flex items-center gap-2 disabled:opacity-30 disabled:cursor-not-allowed text-white"
                        style={{
                    ...
                    <div className="flex gap-2 mt-2.5 flex-wrap items-center">
                      {(aiSuggestions.length > 0 ? aiSuggestions : getDynamicSuggestions(messages, portfolio, keeperResult, connectedAccount, sentiment)).map((q) => (
                        <button
                          key={q}
                          onClick={() => setInput(q)}
                          className="text-[11px] text-purple-400/70 hover:text-purple-300 rounded-lg px-3 py-1.5 transition-all duration-200 border border-purple-800/25 hover:border-purple-600/40"
                      ...
                      {aiSuggestions.length > 0 && (
                        <span className="text-[9px] text-purple-500/50 ml-1 font-medium">✨ AI</span>
                      )}
                    </div>
                  </div>
                </div>
      ...
      {lastTxResult && (
        <div className="fixed bottom-4 right-4 z-50 animate-slide-up">
          <div
            ...
            style={{ background: lastTxResult.success ? "rgba(16, 185, 129, 0.1)" : "rgba(239, 68, 68, 0.1)" }}
          >
            {lastTxResult.success ? (
...
function getDynamicSuggestions(
  ...
  if (lastCharts.includes("portfolio")) return ["Show risk/return scatter", "Run dry run", "Show correlation matrix"];
  if (lastCharts.includes("sentiment")) return ["Run dry run", "Show DeFi heat map", "What should I do?"];
  if (lastCharts.includes("apycompare")) return ["Deposit into highest yield", "Show DeFi heat map", "Explain the risks"];
  if (lastCharts.includes("market")) return ["Deposit 100 HBAR", "Compare Bonzo Vault APYs", "Show DeFi heat map"];
  if (lastCharts.includes("walletinfo")) return ["Show my positions", "Deposit 100 HBAR", "Run dry run"];
  if (lastCharts.includes("strategyconfig")) return ["Run dry run", "Set bearish threshold to -25", "Reset strategy"];
  if (lastCharts.includes("performance")) return ["Run dry run", "Show strategy config", "Compare Bonzo Vault APYs"];
  if (lastCharts.includes("ohlcv")) return ["Show market sentiment", "Run dry run", "What's the trend?"];
  if (lastCharts.includes("heatmap")) return ["Deposit into best opportunity", "Compare APYs", "Show risk/return scatter"];
  if (lastCharts.includes("correlation") || lastCharts.includes("riskreturn")) return ["Show my portfolio", "Run dry run", "Compare Bonzo Vault APYs"];
  if (lastCharts.includes("vaultaction")) return ["Show my positions", "Show audit log", "Compare Bonzo Vault APYs"];
  ...
  if (lastContent.includes("yield") || lastContent.includes("apy")) return ["Deposit into highest yield", "Compare all yields", "Show heat map"];
  if (lastContent.includes("risk") || lastContent.includes("liquidat")) return ["How do I reduce risk?", "Run keeper safety check", "Show health factor"];
  if (lastContent.includes("keeper") || lastContent.includes("autonomous")) return ["Start auto-keeper", "Show decision history", "Show audit log"];
  if (lastContent.includes("vault") || lastContent.includes("clm")) return ["Compare Bonzo Vault APYs", "Switch vault to stable", "Show my positions"];
  if (lastContent.includes("staking")) return ["SAUCE vs HBARX staking?", "Show staking yields", "Compare all APYs"];
...
function KeeperPanel({
  ...
  const actionColors: Record<string, string> = {
    HOLD: "text-yellow-400 border-yellow-500/20",
    HARVEST: "text-red-400 border-red-500/20",
    REPAY_DEBT: "text-red-400 border-red-500/20",
    EXIT_TO_STABLE: "text-red-400 border-red-500/20",
    REBALANCE: "text-blue-400 border-blue-500/20",
    INCREASE_POSITION: "text-emerald-400 border-emerald-500/20",
  };

  const actionBgs: Record<string, string> = {
    HOLD: "rgba(234, 179, 8, 0.08)",
    HARVEST: "rgba(239, 68, 68, 0.08)",
    REPAY_DEBT: "rgba(239, 68, 68, 0.08)",
    EXIT_TO_STABLE: "rgba(239, 68, 68, 0.08)",
    REBALANCE: "rgba(59, 130, 246, 0.08)",
    INCREASE_POSITION: "rgba(16, 185, 129, 0.08)",
  };

  return (
    <div
      className={`rounded-xl border ${justUpdated ? "border-purple-500/40" : "border-purple-900/30"} p-4 transition-colors duration-500`}
      style={{ background: "rgba(15, 12, 30, 0.6)" }}
    >
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2">
          ...
          {justUpdated && (
            <span className="text-[9px] text-purple-400 rounded-full px-2 py-0.5 animate-pulse font-medium" style={{ background: "rgba(139, 92, 246, 0.15)" }}>
              ⚡ Updated
            </span>
          )}
        </div>
      ...
      {/* Auto-loop controls */}
      <div className="flex items-center gap-2 mb-3 p-2.5 rounded-lg border border-purple-900/20" style={{ background: "rgba(20, 16, 40, 0.5)" }}>
        <button
          ...
          className={`flex items-center gap-1.5 text-[11px] font-medium px-2.5 py-1.5 rounded-lg transition-all duration-200 ${
            autoLoop
              ...
              : "text-gray-400 border border-gray-700/30 hover:text-gray-300"
          }`}
          style={autoLoop ? { background: "rgba(139, 92, 246, 0.15)" } : {}}
        >
          {autoLoop ? <Pause className="w-3 h-3" /> : <Timer className="w-3 h-3" />}
          {autoLoop ? "Stop" : "Auto"}
        </button>
        <select
          value={loopInterval}
          onChange={(e) => onSetInterval(Number(e.target.value))}
          className="text-[11px] border border-purple-800/30 rounded-lg px-2 py-1.5 text-gray-300 focus:outline-none"
          style={{ background: "rgba(20, 16, 40, 0.8)" }}
        >
      ...
      <div className="flex gap-2 mb-3">
        <button
          onClick={() => onRun(false)}
          disabled={running}
          className="flex-1 flex items-center justify-center gap-1.5 text-xs font-medium px-3 py-2.5 rounded-xl text-gray-300 border border-purple-800/30 hover:border-purple-600/40 transition-all disabled:opacity-50"
        ...
        <button
          onClick={() => onRun(true)}
          disabled={running}
          className="flex-1 flex items-center justify-center gap-1.5 text-xs font-medium px-3 py-2.5 rounded-xl text-white transition-all disabled:opacity-50"
          style={{ background: running ? "rgba(60, 40, 100, 0.3)" : "linear-gradient(135deg, #7c3aed 0%, #a855f7 100%)" }}
        >
      ...
      {result ? (
        <div className="space-y-2">
          <div className="flex items-center gap-2">
            <div
              className={`text-xs font-medium px-2.5 py-1.5 rounded-lg border inline-block ${
                actionColors[result.decision.action] || "text-gray-400 border-gray-700"
              }`}
              style={{ background: actionBgs[result.decision.action] || "rgba(40, 40, 60, 0.5)" }}
            >
            ...
            {result.hcsLog.logged && (
              <>
                ...
                <span className="text-emerald-400 flex items-center gap-0.5">
                  <CheckCircle className="w-3 h-3" /> HCS
                </span>
              </>
            )}
          </div>
          ...
          {result.vaultDecision && (
            <div className="mt-2 p-2.5 rounded-lg border border-purple-700/25" style={{ background: "rgba(139, 92, 246, 0.05)" }}>
              <div className="flex items-center gap-1.5 mb-1">
                <span className="text-xs">🏦</span>
                <span className="text-[10px] font-medium text-purple-400">Vault Keeper</span>
                <span className={`text-[10px] font-medium px-1.5 py-0.5 rounded ${
                  result.vaultDecision.action === "HOLD" ? "text-yellow-400"
                  : result.vaultDecision.action === "HARVEST" ? "text-orange-400"
                  : result.vaultDecision.action === "DEPOSIT" ? "text-emerald-400"
                  : result.vaultDecision.action === "SWITCH_VAULT" ? "text-blue-400"
                  : "text-gray-400"
                }`} style={{ background: "rgba(139, 92, 246, 0.1)" }}>
        ...
        <p className="text-[11px] text-gray-600">
          Run the keeper to analyze markets and decide on actions. Enable Auto to run continuously.
        </p>
      )}
    </div>
  );
    ...
    return (
      <div className="rounded-xl border border-purple-900/30 p-4" style={{ background: "rgba(15, 12, 30, 0.6)" }}>
        <div className="flex items-center gap-2 mb-3">
          ...
          <h3 className="text-sm font-medium text-gray-400">Loading positions...</h3>
        </div>
        <div className="h-12 animate-pulse rounded-lg" style={{ background: "rgba(139, 92, 246, 0.05)" }} />
      </div>
    );
  }
    ...
    <div className="rounded-xl border border-purple-900/30 p-4" style={{ background: "rgba(15, 12, 30, 0.6)" }}>
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-sm font-medium text-gray-300">Bonzo Positions</h3>
        </div>
      ...
      {!hasPositions ? (
        <div className="text-center py-4">
          ...
          <p className="text-[10px] text-gray-600 mt-1">
            ...
            <a href="https://testnet.bonzo.finance" className="text-purple-400 hover:underline" target="_blank">
        ...
        <div className="space-y-3">
          <div className="grid grid-cols-3 gap-2 text-xs">
            <div className="rounded-lg px-2.5 py-2 border border-purple-900/20" style={{ background: "rgba(20, 16, 40, 0.5)" }}>
              <span className="text-gray-500 text-[10px]">Supplied</span>
              <div className="text-emerald-400 font-semibold">${portfolio.totalSuppliedUSD.toFixed(2)}</div>
            </div>
            <div className="rounded-lg px-2.5 py-2 border border-purple-900/20" style={{ background: "rgba(20, 16, 40, 0.5)" }}>
              <span className="text-gray-500 text-[10px]">Borrowed</span>
              <div className="text-red-400 font-semibold">${portfolio.totalBorrowedUSD.toFixed(2)}</div>
            </div>
            <div className="rounded-lg px-2.5 py-2 border border-purple-900/20" style={{ background: "rgba(20, 16, 40, 0.5)" }}>
              <span className="text-gray-500 text-[10px]">Net APY</span>
              <div className="text-gray-200 font-semibold">{portfolio.averageNetAPY.toFixed(2)}%</div>
            </div>
          </div>
          ...
          {portfolio.positions.map((pos) => (
            <div
              key={pos.symbol}
              className="flex items-center justify-between text-xs px-2.5 py-2 hover:bg-purple-500/5 rounded-lg transition-colors"
            >
              <div className="flex items-center gap-2">
                <span className="text-gray-300 font-medium w-16">{pos.symbol}</span>
                {pos.isCollateral && (
                  <span className="text-[9px] text-purple-400 rounded px-1 py-0.5" style={{ background: "rgba(139, 92, 246, 0.1)" }}>
                    collateral
                  </span>
                )}
              </div>
              <div className="text-right">
                {pos.supplied > 0 && <span className="text-emerald-400">+{pos.supplied.toFixed(4)}</span>}
                {pos.borrowed > 0 && <span className="text-red-400 ml-2">-{pos.borrowed.toFixed(4)}</span>}
              </div>
            </div>
          ))}
        </div>
...
function HealthBadge({ value }: { value: number }) {
  if (!value || value > 1e10) {
    return (
      <span className="text-[10px] font-medium px-2 py-0.5 rounded-full text-emerald-400" style={{ background: "rgba(16, 185, 129, 0.1)" }}>
        HF: ∞ • Safe
      </span>
    );
  }
  ...
  if (value < 1.1) { color = "text-red-400"; bg = "rgba(239, 68, 68, 0.1)"; label = "DANGER"; }
  else if (value < 1.3) { color = "text-orange-400"; bg = "rgba(249, 115, 22, 0.1)"; label = "At Risk"; }
  else if (value < 1.8) { color = "text-yellow-400"; bg = "rgba(234, 179, 8, 0.1)"; label = "Moderate"; }

  return (
    <span className={`text-[10px] font-medium px-2 py-0.5 rounded-full ${color}`} style={{ background: bg }}>
...
function DecisionHistoryCard({ history }: { history: KeeperResult[] }) {
  ...
  const actionIcons: Record<string, string> = {
    HOLD: "🟡", HARVEST: "🔴", REPAY_DEBT: "🔴",
    EXIT_TO_STABLE: "🔴", REBALANCE: "🔵", INCREASE_POSITION: "🟢",
  };
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2 mb-3">
        ...
        <span className="text-[10px] text-gray-500 ml-auto">{history.length} entries</span>
      </div>

      <div className="space-y-2 max-h-48 overflow-y-auto">
        {history.map((entry, i) => (
          <div key={i} className="flex gap-2 text-[11px] py-1.5 border-b border-purple-900/15 last:border-0">
            <span className="flex-shrink-0 mt-0.5">{actionIcons[entry.decision.action] || "⚪"}</span>
...
// ============================================
// Sentiment Card — Purple theme
// ============================================

function SentimentCard({ sentiment, loading }: { sentiment: SentimentData | null; loading: boolean }) {
  if (loading) {
    return (
      <div className="rounded-xl border border-purple-900/30 p-4" style={{ background: "rgba(15, 12, 30, 0.6)" }}>
        <div className="flex items-center gap-2 mb-3">
          <Activity className="w-4 h-4 text-gray-500" />
          <h3 className="text-sm font-medium text-gray-400">Loading sentiment...</h3>
        </div>
        <div className="h-16 animate-pulse rounded-lg" style={{ background: "rgba(139, 92, 246, 0.05)" }} />
      </div>
    );
  }
  ...
  const signalColor =
    sentiment.signal === "HARVEST_NOW" ? "text-red-400"
    : sentiment.signal === "ACCUMULATE" ? "text-emerald-400"
    : "text-yellow-400";

  const SignalIcon =
    sentiment.signal === "HARVEST_NOW" ? TrendingDown
    : sentiment.signal === "ACCUMULATE" ? TrendingUp
    : Minus;
  ...
  return (
    <div className="rounded-xl border border-purple-900/30 p-4" style={{ background: "rgba(15, 12, 30, 0.6)" }}>
      ...
      <div className="mb-3">
        ...
        <div className="h-2 rounded-full overflow-hidden" style={{ background: "rgba(20, 16, 40, 0.8)" }}>
          <div
            className="h-full rounded-full transition-all duration-500"
            style={{
              width: `${((sentiment.score + 100) / 200) * 100}%`,
              background: sentiment.score < -25
      ...
      <div className="grid grid-cols-2 gap-2 text-xs">
        <div className="rounded-lg px-3 py-2 border border-purple-900/20" style={{ background: "rgba(20, 16, 40, 0.5)" }}>
          <span className="text-gray-500">HBAR</span>
          <div className="text-gray-200 font-semibold">${sentiment.dataPoints.hbarPrice.toFixed(4)}</div>
          <span className={sentiment.dataPoints.hbarChange24h >= 0 ? "text-emerald-400" : "text-red-400"}>
            {sentiment.dataPoints.hbarChange24h >= 0 ? "+" : ""}{sentiment.dataPoints.hbarChange24h.toFixed(2)}%
          </span>
        </div>
        <div className="rounded-lg px-3 py-2 border border-purple-900/20" style={{ background: "rgba(20, 16, 40, 0.5)" }}>
          <span className="text-gray-500">Fear & Greed</span>
          <div className="text-gray-200 font-semibold">{sentiment.dataPoints.fearGreedIndex}</div>
          <span className="text-gray-400">{sentiment.dataPoints.fearGreedLabel}</span>
        </div>
      </div>

      {sentiment.dataPoints.volatility !== undefined && (
        <div className="mt-2 rounded-lg px-3 py-2 text-xs border border-purple-900/20" style={{ background: "rgba(20, 16, 40, 0.5)" }}>
          ...
          {sentiment.dataPoints.volatilityTrend && (
            <span className="text-gray-500 ml-1">({sentiment.dataPoints.volatilityTrend})</span>
          )}
        </div>
      )}

      <p className="text-[11px] text-gray-500 mt-2 leading-relaxed">{sentiment.reasoning}</p>
    </div>
  );
}

// ============================================
// Bonzo Vaults Card — Purple theme
// ============================================

function VaultCard({ data, loading }: { data: any; loading: boolean }) {
  if (loading) {
    return (
      <div className="rounded-xl border border-purple-900/30 p-4" style={{ background: "rgba(15, 12, 30, 0.6)" }}>
        <div className="animate-pulse space-y-2">
          <div className="h-4 rounded w-1/2" style={{ background: "rgba(139, 92, 246, 0.1)" }} />
          <div className="h-3 rounded w-full" style={{ background: "rgba(139, 92, 246, 0.05)" }} />
          <div className="h-3 rounded w-3/4" style={{ background: "rgba(139, 92, 246, 0.05)" }} />
        </div>
      </div>
    );
  }
  ...
  const riskColors: Record<string, { text: string; bg: string }> = {
    low: { text: "text-green-400", bg: "rgba(34, 197, 94, 0.1)" },
    medium: { text: "text-yellow-400", bg: "rgba(234, 179, 8, 0.1)" },
    high: { text: "text-red-400", bg: "rgba(239, 68, 68, 0.1)" },
  };
  ...
  return (
    <div className="rounded-xl border border-purple-700/25 p-4" style={{ background: "rgba(15, 12, 30, 0.6)" }}>
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2">
        ...
        <div className="text-right">
          ...
          <div className="text-sm font-bold text-purple-400">${(totalTVL / 1e6).toFixed(2)}M</div>
        </div>
      </div>
      ...
      <div className="space-y-2">
        {vaults.map((vault: any) => (
          <div
            key={vault.id}
            className="flex items-center justify-between px-3 py-2.5 rounded-lg border border-purple-900/20 hover:border-purple-600/30 transition-all"
            style={{ background: "rgba(20, 16, 40, 0.5)" }}
          >
            <div className="flex items-center gap-2 min-w-0">
              <span className="text-sm flex-shrink-0">{strategyIcons[vault.strategy] || "📦"}</span>
              <div className="min-w-0">
                <div className="text-xs font-medium text-gray-200 truncate">{vault.name}</div>
                <div className="text-[10px] text-gray-500">{vault.underlyingProtocol}</div>
              </div>
            </div>
            <div className="flex items-center gap-3 flex-shrink-0">
              <span
                className={`text-[10px] px-1.5 py-0.5 rounded ${riskColors[vault.riskLevel]?.text || "text-gray-400"}`}
                style={{ background: riskColors[vault.riskLevel]?.bg || "rgba(40, 40, 60, 0.5)" }}
              >
                {vault.riskLevel}
              </span>
              <div className="text-right">
                <div className="text-sm font-bold text-emerald-400">{vault.apy?.toFixed(1)}%</div>
      ...
      <div className="mt-3 pt-2 border-t border-purple-900/20 flex justify-between text-xs text-gray-500">
    ...
    return (
      <div className="rounded-xl border border-purple-900/30 p-4" style={{ background: "rgba(15, 12, 30, 0.6)" }}>
        <div className="flex items-center gap-2 mb-3">
          ...
          <h3 className="text-sm font-medium text-gray-400">Loading Bonzo markets...</h3>
        </div>
        <div className="space-y-2">
          {[1, 2, 3].map((i) => (
            <div key={i} className="h-8 animate-pulse rounded" style={{ background: "rgba(139, 92, 246, 0.05)" }} />
          ))}
        </div>
      </div>
    );
  ...
  return (
    <div className="rounded-xl border border-purple-900/30 p-4" style={{ background: "rgba(15, 12, 30, 0.6)" }}>
      <div className="flex items-center justify-between mb-3">
        ...
        <span className="text-[10px] text-gray-500">{activeMarkets.length} active</span>
      </div>

      <div className="space-y-1">
        <div className="grid grid-cols-4 text-[10px] text-gray-600 px-2.5 py-1.5 font-medium uppercase tracking-wider">
          <span>Asset</span>
        ...
        {activeMarkets.slice(0, 8).map((m) => (
          <div
            key={m.symbol}
            className="grid grid-cols-4 text-xs px-2.5 py-2 hover:bg-purple-500/5 rounded-lg transition-colors"
          >
            <span className="text-gray-300 font-medium">{m.symbol}</span>
            <span className="text-right text-emerald-400 font-medium">{m.supplyAPY.toFixed(2)}%</span>
            <span className="text-right text-yellow-400 font-medium">{m.borrowAPY.toFixed(2)}%</span>
            <span className="text-right text-gray-400">{m.utilizationRate.toFixed(1)}%</span>
          </div>
        ))}
      </div>
    </div>
